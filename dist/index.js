...
async function run() {
    ...
    if (util === 'check-files-existence') {
        ...
        await (0, check_files_existence_1.checkFilesExistence)({ files, failure });
    }
    if (util === 'read-file') {
        ...
        await (0, read_file_1.readFile)({ filepath, failure, trim, parse });
    }
}
...
const readFile = async ({ filepath, failure, trim, parse }) => {
    try {
        ...
        if (parse === 'mocha-failure-report') {
            ...
            content = (0, exports.parseFailureReport)(content);
        }
        if (parse === 'json-array') {
            // eslint-disable-next-line no-console
